...
from entities.base import Client
from entities.AdaptiveFedAvg import AdaptiveFedAvgServer
from utils.drift import sudden_drift, incremental_drift
from utils.gen_dataset import distribute_dataset

if __name__ == "__main__":
    # load configuration
    with open("../configs/AdaptiveFedAvg.yaml", 'r') as f:
    ...
    server = AdaptiveFedAvgServer(args)
    ...
    for _round in range(args["rounds"]):
        ...
        client_lr = server.cal_adaptive_lr(_round)
        for client in selected_clients:
            client.lr = client_lr
